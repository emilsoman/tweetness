...
require 'redis'
require 'thread'
require 'lib/tweetness/twitter/twitter_stream'
  ...
  class TwitterReader
    ...
    def detach_flusher
      Thread.new do
        ...
        absolute_start_time = stats_start
        loop do
          sleep(1)
          ...
          if (elapsed = (Time.now.to_i - stats_start)) >= STATS_INTERVAL
            ...
            @redis.set("tweet_count", tweets_count)
            @redis.set("stats_start_time", absolute_start_time)
...
loop do
  begin
    Tweetness::TwitterReader.new.start
  rescue
